...
export default class Pause extends AuthenticatedCommand {
  static override description = 'Pause multiple entities';

  static override examples = [
    '<%= config.bin %> bulk pause --type campaign --ids 123,456,789',
    '<%= config.bin %> bulk pause --type adset --ids 123,456',
  ];
  ...
  async run(): Promise<void> {
    ...
    await this.runWithAuth(this.toFlagValues(flags), async () => {
      ...
      for (const id of ids) {
        try {
          if (flags.type === 'campaign') {
            await this.client.updateCampaignStatus(id, 'PAUSED');
          } else if (flags.type === 'adset') {
            await this.client.updateAdSetStatus(id, 'PAUSED');
          } else if (flags.type === 'ad') {
            await this.client.updateAdStatus(id, 'PAUSED');
          }
      ...
      this.formatter.success(`Paused ${successCount}/${ids.length} ${flags.type}s`);
